...
class ZoomDragManager:
    ...
    def connect(self):
        ...
        self.cid_scroll = self.canvas.mpl_connect(
                'scroll_event', self.on_zoom)
    ...
    def on_press(self, event):
        ...
        self.ismotion = False
        # Consider dragging the image only if the left button was
        # clicked.
        if event.button == 1:
    ...
    def on_motion(self, event):
        ...
        if (np.abs(event.xdata-self.x0)/(xmax-xmin) < 2e-3 and
    ...
    def on_zoom(self, event):
        ...
        if event.button == 1 or event.button == 'up':
            ...
            # size along each dimension; ie new axes limits will span
            # half the length of the current ones.
            dx, dy = xmax-xmin, ymax-ymin
            # The zoomed-in area will be centred about the clicked
            # point.
            ...
            if xmin < 0:
                xmin, xmax = 0, dx*factor
            if xmax > self.xmax:
                xmax = self.xmax
                xmin = xmax - dx*factor
            ymin, ymax = cy - dy*factor/2, cy + dy*factor/2
            if ymin < 0:
                ymin, ymax = 0, dy*factor
            if ymax > self.ymax:
                ymax = self.ymax
                ymin = ymax - dy*factor
        ...
        # Right button pressed/scroll down = zoom out.
            ...
            # The new axes limits will be twice as long in each
            # dimension as the current ones, and centered about the
            # clicked point.
            dx, dy = xmax-xmin, ymax-ymin
